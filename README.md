...
- 微信小程序分包使用 `uni-app` 标准 `subPackages` 机制，根配置维护在根目录 `pages.json`，并由 `vite-plugin-uni-pages` 扫描分包目录生成实际页面清单。
...
- `scripts/config-sync.mjs`：同步根配置到 `src/pages.json`
...
如果是微信小程序分包页面，在这个项目里应当把页面注册到根目录 `pages.json` 的 `subPackages` 中；同时在 `pages.config.ts` 中补充分包根目录，并在 `vite.config.ts` 的 `UniPages({ subPackages: [...] })` 里加入对应源码目录。分包页面本身的样式推荐写在页面内的 `<route>` 块中，实际页面清单由 `vite-plugin-uni-pages` 扫描生成。更完整的步骤见上面的“微信小程序分包”章节。
...
- 分包页面样式优先使用页面内的 `<route>` 块，减少与自动扫描结果冲突的概率。
